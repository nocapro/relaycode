...
import type { FileOperation } from 'relaycode-core';
...
\`\`\`
    ...
    it('should correctly parse a write operation with an explicit patch strategy', () => {
      ...
      const filePath = 'src/utils.ts';
      const { response } = createLLMResponseString(
        [{ type: 'edit', path: filePath, content, strategy: 'standard-diff' }],
        { uuid: testUuid }
      );
      ...
      if (writeOp?.type === 'write') {
        expect(writeOp.patchStrategy).toBe('standard-diff');
    ...
    it('should correctly parse multiple mixed operations', () => {
      const { response } = createLLMResponseString(
        [
          ...
          { type: 'new', path: 'src/new-feature.ts', content: 'diff content', strategy: 'standard-diff' },
        ],
        {
          uuid: testUuid,
      ...
      expect(parsed?.operations).toEqual([
        { type: 'write', path: 'src/main.ts', content: 'console.log("main");', patchStrategy: 'replace' },
        { type: 'delete', path: 'src/to-delete.ts' },
        { type: 'write', path: 'src/new-feature.ts', content: 'diff content', patchStrategy: 'standard-diff' }
      ]);
...
${LLM_RESPONSE_END(testUuid, [{ edit: filePath }])}
      ...
      const parsed = parseLLMResponse(response);
      const operation = parsed?.operations.find((op: FileOperation) => 'path' in op && op.path === filePath);
...
\`\`\`
    ...
    it('should not detect search-replace when <<<<<<< SEARCH appears in the middle of content', () => {
...
console.log(example);
    ...
    it('should not detect search-replace when only start marker is present without end marker', () => {
...
So it should be treated as regular content, not search-replace
            `;
      const block = createFileBlock(filePath, content);
      const response = block + LLM_RESPONSE_END(uuidv4(), [{ edit: filePath }]);
      const parsed = parseLLMResponse(response);
      ...
      expect(op!.type).toBe('write');
      if (op.type === 'write') {
        expect(op.patchStrategy).toBe('replace'); // Should be 'replace', not 'search-replace'
      }
    });
  });
  ...
  describe('from fixtures', () => {
    ...
    it('should correctly parse search-replace.md', async () => {
      const content = await readFixture('search-replace.md');
      ...
      if (op.type === 'write') {
        ...
        expect(op.patchStrategy).toBe('search-replace');
    ...
    it('should correctly parse standard-diff.md', async () => {
      const content = await readFixture('standard-diff.md');
      const parsed = parseLLMResponse(content);
      ...
      if (op.type === 'write') {
        ...
        expect(op.patchStrategy).toBe('standard-diff');
    ...
    it('should correctly parse multiple-ops.md', async () => {
      ...
      expect(parsed?.operations.find(op => op.type === 'write' && 'path' in op && op.path === 'src/main.ts')).toEqual({
        type: 'write',
        path: 'src/main.ts',
      ...
      expect(parsed?.operations.find(op => op.type === 'delete' && 'path' in op && op.path === 'src/utils.ts')).toEqual({
        type: 'delete',
        path: 'src/utils.ts',
      });

      const newOp = parsed?.operations.find((op: FileOperation) => 'path' in op && op.path.includes('New Component'));
      expect(newOp).toBeDefined();
      expect(newOp!.type).toBe('write');
      if (newOp && newOp.type === 'write') {
        expect(newOp.patchStrategy).toBe('standard-diff');
